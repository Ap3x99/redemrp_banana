...
local Bananagroup = GetRandomIntInRange(0, 0xffffff)
print('Bananagroup: ' .. Bananagroup)
...
function EatBanana()
    Citizen.CreateThread(function()
        ...
        PromptSetGroup(EatPrompt, Bananagroup)
...
function CollectBanana()
    Citizen.CreateThread(function()
        ...
        PromptSetGroup(CollectPrompt, Bananagroup)
    ...
    while true do
        ...
        if bush then
            if active == false then
                local BananaGroupName  = CreateVarString(10, 'LITERAL_STRING', "Banana")
                PromptSetActiveGroupThisFrame(Bananagroup, BananaGroupName)
            end
...
function goEat()
    ...
    Wait(2500)
    -- TriggerServerEvent('wwrp_status:eatbanana') -- HERE IS PLACE TO YOU STATUS/BASICNEEDS TRIGGER
...
function goCollect()
    ...
    TaskPlayAnim(playerPed, "mech_pickup@plant@berries", "base", 8.0, -0.5, -1, 0, 0, true, 0, false, 0, false)
    Wait(2300)
    TriggerServerEvent('redemrp_Banana:addBanana')
    active = false
    ClearPedTasks(playerPed)
end
...
RegisterNetEvent('redemrp_banana:EatBanana')
AddEventHandler('redemrp_banana:EatBanana', function()
    local playerPed = PlayerPedId()
    RequestAnimDict("mech_pickup@plant@berries")
    while not HasAnimDictLoaded("mech_pickup@plant@berries") do
        Wait(100)
    ...
    TaskPlayAnim(playerPed, "mech_pickup@plant@berries", "exit_eat", 8.0, -0.5, -1, 0, 0, true, 0, false, 0, false)
    Wait(2500)
    TriggerServerEvent('wwrp_status:eatbanana')
    amount = amount + 1
    --[[
    if amount == 4  then
        TriggerEvent("redem_roleplay:Tip", "Do not eat blueberries too often, this can result in poisoning!", 4000)
...
function GetClosestBush()
    ...
    if size > 0 then
        for index = 0, size - 1 do
            ...
            if (model_hash ==  1950330915 or model_hash ==  -1015354716 or model_hash ==  -987417712) and not oldBush[tostring(entity)] then
